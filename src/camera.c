...
/*   Updated: 2024/09/10 00:29:58 by senayat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera	*camera_init(t_env_fdf *env)
{
	...
	camera->z_height = 1.0;
	camera->zoom = (int)fmin((float)SCREEN_WIDTH / env->map->size.x / 2,
			(float)SCREEN_HEIGHT / env->map->size.y / 2);
	camera->zoom_init = camera->zoom;
	return (camera);
}
...
t_bool	change_perspective(t_camera *camera)
{
	...
	{
		camera->view = SIDE;
		change_angles(camera, -DEG_45_RAD - DEG_45_RAD, 0.0, 0.0);
	}
	else if (camera->view == SIDE)
	{
		camera->view = TOPDOWN;
